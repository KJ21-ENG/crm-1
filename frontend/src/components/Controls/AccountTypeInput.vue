...
async function confirmAndDelete(option, evt) {
  try {
        ...
        <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div class="w-full max-w-sm rounded-lg bg-white shadow-xl">
            ...
            <div class="px-4 py-3 flex justify-end gap-2 border-t">
              <button id="at-cancel" class="px-3 py-1.5 text-sm rounded border">${__('Cancel')}</button>
              <button id="at-ok" class="px-3 py-1.5 text-sm rounded bg-red-600 text-white">${__('Delete')}</button>
            </div>
          </div>
        </div>`
      document.body.appendChild(confirmDialog)
      confirmDialog.querySelector('#at-cancel').addEventListener('click', () => {
        document.body.removeChild(confirmDialog)
        resolve(false)
      })
      confirmDialog.querySelector('#at-ok').addEventListener('click', () => {
        document.body.removeChild(confirmDialog)
        resolve(true)
      })
    })
    ...
    // If the deleted option is currently selected, clear it
