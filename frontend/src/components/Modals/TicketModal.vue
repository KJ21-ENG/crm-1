                  ...
                  <div v-if="customerHistory.data.summary" class="pt-4 border-t">
                    ...
                    <div class="text-sm space-y-1">
                      ...
                      <div v-if="customerHistory.data.summary.last_interaction">
                        <strong>Last Contact:</strong> {{ formatDate(customerHistory.data.summary.last_interaction, 'DD/MM/YYYY') }}
                      </div>
                    </div>
                  </div>
...
const tabs = createResource({
  ...
  transform: (_tabs) => {
    _tabs.forEach((tab) => {
      // Restructure Contact Information: keep only First Name, Last Name, Mobile No
      // and move the rest into a collapsible expander titled "Additional Contact Information"
      ...
      if (contactSectionIndex !== -1) {
        ...
        // Gather all fields from all columns in order
        const allFields = []
        ;(contactSection.columns || []).forEach((col) => {
          ;(col.fields || []).forEach((f) => allFields.push(f))
        })

        // Split fields into kept and additional
        ...
        // Rebuild the main contact section with only kept fields in three columns (First, Last, Mobile)
        const colNames = [
        ...
        // If there are additional fields, insert a new collapsible section right after
        if (additionalFields.length) {
          // Distribute additional fields across three columns in round-robin
