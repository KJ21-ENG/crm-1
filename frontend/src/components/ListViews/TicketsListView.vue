...
<template>
          ...
          <div v-else-if="column.key === 'priority'">
            <div 
              ...
              :class="{
                ...
                'bg-orange-500': item.value === 'Medium', 
                'bg-red-500': item.value === 'High',
                'bg-purple-500': item.value === 'Urgent'
              }"
            />
          </div>
  ...
  />
  <ListBulkActions ref="listBulkActionsRef" v-model="list" doctype="CRM Ticket" />
</template>
...
const currentPage = computed(() => {
  ...
  console.log('🔍 TicketsListView Debug - Current page calculation:', {
    start,
    pageLength,
    calculatedPage,
